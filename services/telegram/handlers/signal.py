...
async def signal_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Only executable directional decisions may enter the tracker. Any
        # neutral/legacy/unknown status is deliberately treated as non-tradable.
        if user_id is not None and str(report.signal).upper() in {"BUY", "SELL"}:
            track_report(user_id, symbol, timeframe, report)
        await status_message.edit_text(
            _format_signal(report, symbol, timeframe),
            parse_mode="HTML",
        )
