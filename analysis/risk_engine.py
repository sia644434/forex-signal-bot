...
class RiskEngine:
    ...
    def __init__(self, risk_reward_target: float = 2.0, atr_multiplier: float = 1.5, account_balance: float = 1000.0, risk_percent: float = 1.0, contract_size: float | None = None, account_currency: str | None = None) -> None:
        ...
        if self.account_currency is not None and not ((len(self.account_currency) == 3 and self.account_currency.isalpha()) or self.account_currency in {"USDT", "USDC"}):
            raise ValueError("account_currency must be a supported currency code (3-letter ISO code or USDT/USDC).")
    ...
    def _calculate_position_size(self, risk_distance: float, dynamic_risk_percent: float, *, symbol: str | None, quote_to_account_rate: float | None) -> tuple[float | None, float | None, float | None, str | None]:
        ...
        try:
            ...
            contract_size = self.contract_size if self.contract_size is not None else get_contract_size(symbol)
