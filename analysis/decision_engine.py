...
class DecisionEngine:
    ...
    def __init__(
        ...
        self.strong_buy_threshold = self._require_finite(
            strong_buy_threshold, "strong_buy_threshold"
        )
        self.strong_sell_threshold = self._require_finite(
    ...
    @classmethod
    def _read_component(
        cls, analysis: Any, attribute: str, default: float = 0.0
    ) -> float:
    ...
    def _apply_smart_money(self, total: float, analysis: Any, reasons: list[str]) -> float:
        total = self._apply_component(
            total, self._read_component(analysis, "smart_money_score"),
            self.weights["smart_money"], reasons, "Smart Money",
            "Smart Money Concepts support a bullish scenario",
            "Smart Money Concepts support a bearish scenario",
            "Smart Money Concepts are currently neutral",
        )
        smc_bias = getattr(analysis, "smc_bias", "neutral")
        if isinstance(smc_bias, str):
            smc_bias = smc_bias.lower().strip()
            if smc_bias == "bullish":
                reasons.append("SMC bias is bullish")
            elif smc_bias == "bearish":
                reasons.append("SMC bias is bearish")
        return total

    def _apply_structure(self, total: float, analysis: Any, reasons: list[str]) -> float:
        combined_score = (
            self._read_component(analysis, "structure_score")
            + self._read_component(analysis, "trend_score")
        ) / 2.0
        total = self._apply_component(
            total, combined_score, self.weights["structure"], reasons, "Market Structure",
            "Market structure favors buyers", "Market structure favors sellers",
            "Market structure is balanced",
        )
        trend = getattr(analysis, "trend", "neutral")
        if isinstance(trend, str):
            trend = trend.lower().strip()
            if trend == "bullish":
                reasons.append("Higher-level market trend is bullish")
            elif trend == "bearish":
                reasons.append("Higher-level market trend is bearish")
        return total

    def _apply_price_action(self, total: float, analysis: Any, reasons: list[str]) -> float:
        return self._apply_component(
            total, self._read_component(analysis, "price_action_score"),
            self.weights["price_action"], reasons, "Price Action",
            "Price action confirms bullish pressure", "Price action confirms bearish pressure",
            "Price action does not provide a strong directional edge",
        )

    def _apply_supply_demand(self, total: float, analysis: Any, reasons: list[str]) -> float:
        total = self._apply_component(
            total, self._read_component(analysis, "supply_demand_score"),
            self.weights["supply_demand"], reasons, "Supply Demand",
            "Supply/Demand conditions favor demand", "Supply/Demand conditions favor supply",
            "Supply/Demand conditions are neutral",
        )
        supply_demand = getattr(analysis, "supply_demand", None)
        if supply_demand:
            reasons.append(f"Supply/Demand zone: {supply_demand}")
        return total

    def _apply_indicators(self, total: float, analysis: Any, reasons: list[str]) -> float:
        return self._apply_component(
            total, self._read_component(analysis, "momentum_score"),
            self.weights["indicators"], reasons, "Indicators",
            "Momentum and indicators support buyers", "Momentum and indicators support sellers",
            "Momentum is not strongly directional",
        )

    def _apply_candlestick(self, total: float, analysis: Any, reasons: list[str]) -> float:
        return self._apply_component(
            total, self._read_component(analysis, "candlestick_score"),
            self.weights["candlestick"], reasons, "Candlestick",
    ...
    def _apply_elliott(self, total: float, analysis: Any, reasons: list[str]) -> float:
        return self._apply_component(
            total, self._read_component(analysis, "elliott_score"),
            self.weights["elliott"], reasons, "Elliott",
            "Elliott analysis favors bullish structure", "Elliott analysis favors bearish structure",
            "Elliott wave structure is inconclusive",
        )

    def _apply_harmonic(self, total: float, analysis: Any, reasons: list[str]) -> float:
        return self._apply_component(
            total, self._read_component(analysis, "harmonic_score"),
            self.weights["harmonic"], reasons, "Harmonic",
            "Harmonic analysis supports bullish conditions", "Harmonic analysis supports bearish conditions",
            "No strong harmonic directional confirmation",
        )

    def _apply_brooks(self, total: float, analysis: Any, reasons: list[str]) -> float:
        return self._apply_component(
            total, self._read_component(analysis, "brooks_score"),
            self.weights["brooks"], reasons, "Brooks",
            "Brooks price-action analysis favors bulls", "Brooks price-action analysis favors bears",
            "Brooks analysis is currently balanced",
        )

    def _apply_wyckoff(self, total: float, analysis: Any, reasons: list[str]) -> float:
        return self._apply_component(
            total, self._read_component(analysis, "wyckoff_score"),
            self.weights["wyckoff"], reasons, "Wyckoff",
            "Wyckoff structure favors accumulation/markup", "Wyckoff structure favors distribution/markdown",
    ...
    def _calculate_final_score(self, analysis: Any, reasons: list[str]) -> float:
        score = 0.0
        for method in (
            self._apply_smart_money,
            self._apply_structure,
            self._apply_price_action,
            self._apply_supply_demand,
            self._apply_indicators,
            self._apply_candlestick,
            self._apply_elliott,
            self._apply_harmonic,
            self._apply_brooks,
            self._apply_wyckoff,
        ):
    ...
    def _calculate_strength(self, score: float) -> str:
        if score >= self.strong_buy_threshold or score <= self.strong_sell_threshold:
            return "STRONG"
        if score >= self.buy_threshold or score <= self.sell_threshold:
            return "MODERATE"
        return "WEAK"
    ...
    def _build_final_reasons(
        reasons: list[str], score: float, confidence: float,
        signal: str, strength: str, bias: str,
    ) -> list[str]:
    ...
    def decide(self, analysis: Any) -> DecisionResult:
        ...
        reasons = self._build_final_reasons(
            reasons, score, confidence, signal, strength, bias
        )
